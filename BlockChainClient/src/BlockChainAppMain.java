...
public class BlockChainAppMain<UserObject> extends JFrame {
	...
	public BlockChainAppMain() {
		...
		separator.setPreferredSize(new Dimension(238, 343));
	...
	public void init_textArea() { // 初始化區塊區資訊與按鈕功能
		JTextArea areaBlockData = new JTextArea();
		areaBlockData.setPreferredSize(new Dimension(-10, 480)); 
		areaBlockData.setMinimumSize(new Dimension(-10, 480)); 
		areaBlockData.setMaximumSize(new Dimension(-10, 480)); 

		JScrollPane textscrollPane = new JScrollPane(areaBlockData);
		JPanel buttonPanel = new JPanel();
		buttonPanel.setLayout(new BoxLayout(buttonPanel, BoxLayout.Y_AXIS));
		JButton areaNewButton = new JButton("查詢最新區塊");
		areaNewButton.addActionListener(e -> btn_Click("blockNew", "", "GET", areaBlockData));
		JButton areaAllButton = new JButton("查詢所有區塊");
		areaAllButton.addActionListener(e -> btn_Click("blockAll", "", "GET", areaBlockData));
		...
		TimerTask task = new TimerTask() {
			@Override
			public void run() {
				updateData(areaBlockData); // 更新 JTextArea 的內容
			}
		};

		// 設定延遲時間 2000 毫秒 (2 秒)，並每隔 2000 毫秒執行一次
		timer.schedule(task, 1500, 1500);
	}

	private void updateData(JTextArea areaBlockData) {
		if (!httpMethod.result.equals("")) {

			areaBlockData.setText(httpMethod.result);

		} else {
			areaBlockData.setText("Loading....");

		}
		return;
	}

	public void btn_Click(String btnName, String Value, String Method, JTextArea areaBlockData) {
		String Url = "";
		if (btnName.equals("blockNew")) {
			Url = httpMethod.routerSelect(HttpMethod.BlockRouter.BLOCKNEW);
		}
		;
		if (btnName.equals("blockAll")) {
			Url = httpMethod.routerSelect(HttpMethod.BlockRouter.BLOCKALL);
		}
		;
		if (btnName.equals("blockNews")) {
			Url = httpMethod.routerSelect(HttpMethod.BlockRouter.BLOCKNEW);
		}
		;
		if (btnName.equals("blockNews")) {
		...
		areaBlockData.setText(content);

	}

	public void init_MenuBar(JMenuBar Tools, String Item) {
		if (Item.equals("功能選單")) {
			creat_Menu(Tools, new String[] { "管理員視窗","功能1", "功能2", "功能3" }, "功能選單");
	...
	private void creat_Menu(JMenuBar menu, String[] Items, String MenuName) { // 初始化menu
		...
		for (String menuItem : Items) {
			...
			menus.add(Item);
			Item.addActionListener(e -> creat_MenuWindow("管理員視窗", e.getActionCommand()));
		}

	}

	private void creat_MenuWindow(String MenuName, String MenuFunciont) { // 選擇開啟視窗
        if(MenuName.equals("管理員視窗")) {
        	BlockChainAdmin window=new BlockChainAdmin();
        	window.show();
        }else {
